...
public class SendingService {
	...
	private static ConcurrentHashMap<Long, RecipeReply> cacheMap = new ConcurrentHashMap<>();
	...
	public void sendReply(RecipeTelegramBot telegramBot, Reply reply) {
		...
		if (reply instanceof RecipeReply) {
			...
			if (CollectionUtils.isEmpty(recipes)) {
				String errorMessage = "Мне не удалось найти информацию по Вашему запросу =( Пожалуйста, попробуйте еще раз";
			...
			String errorMessage = "Упс... Что-то пошло не так. Боту не удалось распознать Ваш запрос. Пожалуйста, попробуйте еще раз";
			sendMessage.setText(errorMessage);
		}
	...
	public void sendSelectedRecipeFromCache(RecipeTelegramBot telegramBot, Long chatId, String request) {
		...
		File file = new File("images/" + recipe.getImage());
		...
		sendMessage.setText(recipe.toString());
		send(telegramBot, sendMessage);
	}
