...
public class ParsedWebPage {
	...
	private String parseElementByClass(Document doc, String elementClass, boolean deleteLastWord) {
		String result = null;
		try {
			...
			if (elements.size() > 0) {
				...
				if (StringUtils.isEmpty(result)) {
					log.info(String.format("Параметр [%s] не был получен при парсинге html страницы [%s]", elementClass, pageURL));
				}
			}
		} catch (Exception e) {
			log.error(String.format("Ошибка получения параметра [%s] при парсинге html страницы [%s]", elementClass, pageURL), e);
		}
		return result;
	}

	private List<RecipeIngredientDto> parseRecipeIngredients(Document doc, String elementClass) {
	...
	private List<RecipeInstructionDto> parseRecipeInstructions(Document doc, String elementClass) {
	...
	private String saveImageAndReference(Document doc, String imageClass) {
		String fileName = null;
		try {
			Elements elements = doc.getElementsByClass(imageClass);
			...
			log.error(String.format("Ошибка сохранения изображения по параметру [%s] при парсинге html страницы [%s]", imageClass, pageURL), e);
			return null;
		}
		return fileName;
	}
